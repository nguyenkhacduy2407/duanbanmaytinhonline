...
const sendEmail = async ({ to, subject, text, html, ...rest }) => {
  try {
    ...
    if (res) {
      ...
      const mail = {
        //sender access
        from: '"KD&&TT" <no-reply@accounts.kd&&tt.com>',
        //receiver access
        to,
        //subject
        subject,
...
const footerHtmlVerifyMail = `<h3 style="color: 	#0000FF">
...
const htmlSignupAccount = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào ,<br />
        Mã xác nhận đăng ký tài khoản cho website KD&&TT của anh (chị).<br />
        Cảm ơn vì đã ghé thăm KD&&TT <3
    </h2>
...
const htmlResetPassword = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào ,<br />
        Cửa hàng KD&&TT đã nhận được yêu cầu lấy lại mật khẩu từ quý khách.<br />
       Nhập mã xác nhận để phôi phục mật khẩu...
    </h2>
